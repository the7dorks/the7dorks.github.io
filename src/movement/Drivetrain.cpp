...
std::shared_ptr<ChassisController> Drivetrain::mchassis =
    ChassisControllerBuilder()
        ...
        .withDimensions(AbstractMotor::gearset::green,
                        {{def::DRIVE_STRAIGHT_SCALE, def::DRIVE_TURN_SCALE}, imev5GreenTPR})
 ...
 */
void Drivetrain::tankToPoint(
    ExtendedPoint itarget, std::vector<AsyncAction> iactions, double imax, bool ireversed, double iturnWeight,
                    ...
                    // the AsyncActions at the right times
{
    ...
    {
        QAngle angleToPoint = util::wrapQAngle180(Drivetrain::angleToPoint(itarget) -
                                                  (Drivetrain::getTheta() +
                                                   (ireversed ? M_PI : 0) *
                                                       radian)); // how much the robot needs to turn to face the point

        // def::display.setMiscData(1, "angToPnt: " + std::to_string(Drivetrain::angleToPoint(itarget).convert(degree)) + "\ntheta: " + std::to_string(Drivetrain::getTheta().convert(degree)) + "\npreWrap: " + std::to_string((Drivetrain::angleToPoint(itarget) - (Drivetrain::getTheta() + (ireversed ? M_PI : 0) * radian)).convert(degree)) + "\nturnToPoint: " + std::to_string(angleToPoint.convert(degree)));
        ...
        Drivetrain::moveArcade(forward * (ireversed ? -1 : 1), turn, true);
        // def::display.setMiscData(1, "angToPoint: " + std::to_string(angleToPoint.convert(degree)) + "\ninToPoint: " + std::to_string(inToPoint) + "\ninForward: " + std::to_string(inForward));
...
void Drivetrain::straightForDistance(QLength idistance, std::vector<AsyncAction> iactions,
                                     PID imagnitudePID, Slew imagnitudeSlew, PID iturnPID)
{
...
void Drivetrain::followPathfinder(const std::string &iname, bool isettle, bool ibackwards,
                                  bool imirrored) // follow Pathfinder path through okapi
{
    mprofiler->setTarget(iname, ibackwards, imirrored);
    if (isettle)
    {
        mprofiler->waitUntilSettled();
    }
}
