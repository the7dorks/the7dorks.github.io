    ...
    lv_obj_t *idropdown) // specifies the code to be executed when the auton dropdown is changed
{
                                   ...
                                   // its contents
    {
        ...
        {
            fprintf(file, "%i",
                    (lv_ddlist_get_selected(mtabview_auton_dropdown) * 100 + lv_ddlist_get_selected(mtabview_auton_dropdown_side) * 10 + lv_ddlist_get_selected(mtabview_auton_dropdown_color))); // update sd card based on new value
            std::cout << "set /usd/auton_settings.txt to "
                      << std::to_string(lv_ddlist_get_selected(mtabview_auton_dropdown) * 100 + lv_ddlist_get_selected(mtabview_auton_dropdown_side) * 10 + lv_ddlist_get_selected(mtabview_auton_dropdown_color))
                      << std::endl;
        }
        else
        {
