...
public:
    ...
    double iterate(double ivalue); // limits the input value to maximum changes described by
 ...
 */
    ...
    */
    ...
    scaleToFit(double imagnitude,
               std::array<double, N> &&
                   iarray) // scales all elements in iarray to fit within [-imagnitude, imagnitude]
    {
    ...
    template <std::size_t N>
    void scaleToFit(double imagnitude,
                    std::array<double, N> &
                        iarray) // scales all elements in iarray to fit within [-imagnitude, imagnitude]
    {
        double largest = findAbsMax<double, N>(iarray);
    ...
    template <class T, std::size_t N>
    void chop(T imin, T imax,
              std::array<T, N> &
                  iarray) // if any values in iarray are out of range, they are set to the limit
    {
